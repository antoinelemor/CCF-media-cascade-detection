#!/usr/bin/env python3
"""
PROJECT: CCF-media-cascade-detection
TITLE: fig_phase1_impact_by_frame_2018.py

Generates Phase 1 impact analysis figures for ALL 8 frames (2018).
For each frame, focuses on the top 2 cascades (by total_score) and shows
all their significant event clusters.

Panel A — Timeline: daily frame signal with top-2 cascade windows and
          cluster peaks annotated by role (driver / late_support / suppressor).
Panel B — Horizontal lollipop chart: impact score by cluster, coloured
          by role, with auto-generated event descriptions.

Author: Antoine Lemor
"""

import pickle
import sys
import warnings
from pathlib import Path

import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
import matplotlib.dates as mdates
import matplotlib.patheffects as PathEffects
import numpy as np
import pandas as pd

warnings.filterwarnings('ignore')

PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
sys.path.insert(0, str(PROJECT_ROOT))

from cascade_detector.core.constants import FRAMES, FRAME_COLUMNS, FRAME_COLORS
from cascade_detector.analysis.unified_impact import UnifiedImpactAnalyzer

# ---------------------------------------------------------------------------
# Style
# ---------------------------------------------------------------------------
plt.rcParams.update({
    'font.family': 'serif',
    'font.serif': ['Times New Roman', 'DejaVu Serif'],
    'mathtext.fontset': 'dejavuserif',
    'axes.titlesize': 13,
    'axes.labelsize': 11,
    'xtick.labelsize': 10,
    'ytick.labelsize': 10,
    'legend.fontsize': 9,
})

ROLE_COLORS = {
    'driver': '#27AE60',
    'late_support': '#F39C12',
    'suppressor': '#E74C3C',
}

FRAME_FULL_NAMES = {
    'Cult': 'Cultural',
    'Eco': 'Economic',
    'Envt': 'Environmental',
    'Pbh': 'Public Health',
    'Just': 'Justice',
    'Pol': 'Political',
    'Sci': 'Scientific',
    'Secu': 'Security',
}

# Distinct cascade band colors (cycle for frames with many cascades)
CASCADE_PALETTE = [
    ('#2E86C1', '#1A5276'),
    ('#D4AC0D', '#7D6608'),
    ('#C0392B', '#922B21'),
    ('#27AE60', '#1E8449'),
    ('#8E44AD', '#6C3483'),
    ('#E67E22', '#A04000'),
    ('#1ABC9C', '#148F77'),
    ('#2C3E50', '#1B2631'),
]


# ---------------------------------------------------------------------------
# Data loading
# ---------------------------------------------------------------------------
def load_data():
    cache_path = PROJECT_ROOT / 'results' / 'cache' / 'results_2018.pkl'
    with open(cache_path, 'rb') as f:
        results = pickle.load(f)

    # Use cached impact results (already computed by rerun_impact_2018.py)
    impact = results.event_impact
    if not hasattr(impact, 'cluster_cascade'):
        print("Cache contains legacy impact format — re-running...")
        analyzer = UnifiedImpactAnalyzer()
        impact = analyzer.run(results)
    cc = impact.cluster_cascade

    cluster_map = {c.cluster_id: c for c in results.event_clusters}
    cascade_map = {c.cascade_id: c for c in results.cascades}
    articles = getattr(results, '_articles', pd.DataFrame())

    return cc, cluster_map, cascade_map, articles


# ---------------------------------------------------------------------------
# Auto-generate event description from cluster metadata
# ---------------------------------------------------------------------------
def get_cluster_description(cluster, articles_df):
    """Build a short description from the cluster's top entities and type."""
    parts = []

    # Event type(s)
    if hasattr(cluster, 'event_types') and cluster.event_types:
        type_str = ', '.join(sorted(cluster.event_types)[:2])
        type_str = type_str.replace('evt_', '').replace('_', ' ').title()
        parts.append(type_str)

    # Top entities
    if hasattr(cluster, 'entities') and cluster.entities:
        if isinstance(cluster.entities, dict):
            top_ents = list(cluster.entities.keys())[:3]
        else:
            top_ents = list(cluster.entities)[:3]
        parts.append(', '.join(str(e) for e in top_ents))

    if parts:
        desc = ' — '.join(parts)
    else:
        desc = f"Cluster {cluster.cluster_id}"

    # Truncate to keep labels readable
    if len(desc) > 65:
        desc = desc[:62] + '...'
    return desc


# ---------------------------------------------------------------------------
# Build cascade labels & colors for a given frame
# ---------------------------------------------------------------------------
def build_cascade_style(cascade_ids):
    """Assign colors and short labels to each cascade of a frame."""
    cascade_ids_sorted = sorted(cascade_ids)
    labels = {}
    colors = {}
    dark_colors = {}

    for i, cid in enumerate(cascade_ids_sorted):
        # Extract month from cascade_id (format: Frame_YYYYMMDD_N)
        parts = cid.split('_')
        if len(parts) >= 2:
            try:
                date_str = parts[1]
                month = pd.Timestamp(date_str).strftime('%b')
            except Exception:
                month = f"#{i+1}"
        else:
            month = f"#{i+1}"

        frame_abbr = parts[0] if parts else cid[:4]
        label = f"{frame_abbr} #{i+1} ({month})"
        labels[cid] = label

        pal_idx = i % len(CASCADE_PALETTE)
        colors[cid] = CASCADE_PALETTE[pal_idx][0]
        dark_colors[cid] = CASCADE_PALETTE[pal_idx][1]

    return labels, colors, dark_colors


# ---------------------------------------------------------------------------
# Panel A — Timeline
# ---------------------------------------------------------------------------
def plot_timeline(ax, frame_abbr, significant, cluster_map, cascade_map,
                  articles, cascade_labels, cascade_colors, cascade_dark,
                  dim_cascade_ids=None):
    frame_col_name = FRAME_COLUMNS[frame_abbr]  # e.g. 'health_frame'
    frame_col_mean = f'{frame_col_name}_mean'
    frame_color = FRAME_COLORS[frame_abbr]
    dim_cascade_ids = dim_cascade_ids or set()

    date_col = 'date_converted_first'
    if frame_col_mean in articles.columns:
        use_col = frame_col_mean
    elif frame_col_name in articles.columns:
        use_col = frame_col_name
    else:
        ax.text(0.5, 0.5, f'Column {frame_col_name} not found',
                transform=ax.transAxes, ha='center')
        return

    arts = articles.copy()
    arts['_date'] = pd.to_datetime(arts[date_col], errors='coerce')
    daily = arts.groupby('_date')[use_col].mean()
    daily = daily.loc['2018-01-01':'2018-12-31']
    smoothed = daily.rolling(7, center=True, min_periods=1).mean()

    ax.fill_between(smoothed.index, 0, smoothed.values,
                    color=frame_color, alpha=0.10, zorder=1)
    ax.plot(smoothed.index, smoothed.values,
            color=frame_color, lw=1.4, alpha=0.6, zorder=2)

    # Cascade windows
    cascade_order = sorted(cascade_labels.keys())
    for cid in cascade_order:
        if cid not in cascade_map:
            continue
        cascade = cascade_map[cid]
        onset = pd.Timestamp(cascade.onset_date)
        end = pd.Timestamp(cascade.end_date)
        peak = pd.Timestamp(cascade.peak_date)

        # Dimmed style for cascades without significant clusters
        if cid in dim_cascade_ids:
            ax.axvspan(onset, end, alpha=0.04, color='#95A5A6', zorder=0)
            ax.axvline(peak, color='#95A5A6', ls=':', lw=1.0, alpha=0.4, zorder=3)
            ymax = smoothed.max() * 1.08
            ax.text(peak, ymax, f"peak {peak.strftime('%b %d')}",
                    ha='center', va='bottom', fontsize=7, fontstyle='italic',
                    color='#95A5A6',
                    path_effects=[PathEffects.withStroke(linewidth=2.5, foreground='white')])
        else:
            col = cascade_colors[cid]
            dark = cascade_dark[cid]
            ax.axvspan(onset, end, alpha=0.07, color=col, zorder=0)
            ax.axvline(peak, color=col, ls='--', lw=1.2, alpha=0.5, zorder=3)
            ymax = smoothed.max() * 1.08
            ax.text(peak, ymax, f"peak {peak.strftime('%b %d')}",
                    ha='center', va='bottom', fontsize=8, fontweight='bold',
                    color=dark,
                    path_effects=[PathEffects.withStroke(linewidth=2.5, foreground='white')])

    # Cluster peaks
    for _, row in significant.iterrows():
        cl = cluster_map.get(row['cluster_id'])
        if cl is None:
            continue
        peak_dt = pd.Timestamp(cl.peak_date)
        role = row['role']
        color = ROLE_COLORS[role]

        if peak_dt in smoothed.index:
            y_base = smoothed.loc[peak_dt]
        else:
            idx_pos = smoothed.index.searchsorted(peak_dt)
            idx_pos = min(idx_pos, len(smoothed) - 1)
            y_base = smoothed.iloc[idx_pos] if len(smoothed) > 0 else 0

        marker = {'driver': '^', 'late_support': 'D', 'suppressor': 'v'}[role]
        ax.scatter(peak_dt, y_base, marker=marker, s=65, color=color,
                   edgecolor='white', linewidth=0.8, zorder=5)

    frame_label = frame_col_name.replace('_', ' ')
    ax.set_ylabel(f'Mean {frame_label} score')
    ax.set_xlim(pd.Timestamp('2018-01-01'), pd.Timestamp('2018-12-31'))
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
    ax.xaxis.set_major_locator(mdates.MonthLocator())
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.set_title(
        f'A.  Daily {frame_label.replace("_", " ")} signal with {frame_abbr} '
        f'cascade windows and event cluster peaks',
        loc='left', fontweight='bold', fontsize=11,
    )

    handles = [
        plt.Line2D([0], [0], marker='^', color='w',
                   markerfacecolor=ROLE_COLORS['driver'],
                   markersize=9, label='Driver'),
        plt.Line2D([0], [0], marker='D', color='w',
                   markerfacecolor=ROLE_COLORS['late_support'],
                   markersize=8, label='Late Support'),
        plt.Line2D([0], [0], marker='v', color='w',
                   markerfacecolor=ROLE_COLORS['suppressor'],
                   markersize=9, label='Suppressor'),
        mpatches.Patch(facecolor=frame_color, alpha=0.15,
                       label=f'{frame_label.title()} (7d avg)'),
    ]
    if dim_cascade_ids:
        handles.append(mpatches.Patch(facecolor='#95A5A6', alpha=0.15,
                                      label='Cascade (no significant impact)'))
    ax.legend(handles=handles, loc='upper right', frameon=True, fancybox=True,
              edgecolor='#BDC3C7', fontsize=8.5)


# ---------------------------------------------------------------------------
# Panel B — Lollipop
# ---------------------------------------------------------------------------
def plot_lollipop(ax, frame_abbr, significant, cluster_map, cascade_map,
                  articles, cascade_labels, cascade_colors, cascade_dark):
    frame_col_name = FRAME_COLUMNS[frame_abbr]
    # Only cascades that have significant clusters appear in Panel B
    cascade_order = sorted(significant['cascade_id'].unique())

    rows = []
    for cid in cascade_order:
        sub = significant[significant['cascade_id'] == cid].sort_values(
            'impact_score', ascending=True
        )
        for _, r in sub.iterrows():
            rows.append(r)
    if not rows:
        ax.text(0.5, 0.5, 'No significant clusters for this frame',
                transform=ax.transAxes, ha='center', fontsize=12)
        return

    n = len(rows)

    # Group boundaries
    group_bounds = {}
    current_cascade = None
    for i, row in enumerate(rows):
        cid = row['cascade_id']
        if cid != current_cascade:
            if current_cascade is not None:
                group_bounds[current_cascade] = (group_bounds[current_cascade][0], i - 1)
            group_bounds[cid] = (i, None)
            current_cascade = cid
    group_bounds[current_cascade] = (group_bounds[current_cascade][0], n - 1)

    # Background bands
    for cid in cascade_order:
        if cid in group_bounds:
            y0, y1 = group_bounds[cid]
            ax.axhspan(y0 - 0.5, y1 + 0.5, alpha=0.05,
                       color=cascade_colors[cid], zorder=0)

    # Lollipops
    labels = []
    for i, row in enumerate(rows):
        cl = cluster_map.get(row['cluster_id'])
        role = row['role']
        color = ROLE_COLORS[role]
        impact = row['impact_score']

        # Stem
        ax.plot([0, impact], [i, i], color=color, lw=2.0, alpha=0.55, zorder=2,
                solid_capstyle='round')

        # Dot
        marker = {'driver': '^', 'late_support': 'D', 'suppressor': 'v'}[role]
        ax.scatter(impact, i, marker=marker, s=90, color=color,
                   edgecolor='white', linewidth=1.0, zorder=3)

        # Label
        if cl is not None:
            desc = get_cluster_description(cl, articles)
            peak_str = cl.peak_date.strftime('%b %d')
        else:
            desc = '?'
            peak_str = '?'
        label = f"C{row['cluster_id']}  {desc}  [{peak_str}]"
        labels.append(label)

        # Right annotation: DID + frame affinity
        frame_short = frame_col_name.split('_')[0]  # 'health', 'economic', etc.
        did_sign = '+' if row['diff_in_diff'] > 0 else ''
        ann = (f"DID {did_sign}{row['diff_in_diff']:.3f}   "
               f"{frame_short}={row['frame_affinity']:.3f}")
        ax.text(impact + 0.004, i, ann, ha='left', va='center', fontsize=7.5,
                color='#566573', fontstyle='italic',
                path_effects=[PathEffects.withStroke(linewidth=2, foreground='white')])

    # Y-axis labels with role-aware coloring
    ax.set_yticks(range(n))
    ax.set_yticklabels(labels, fontsize=8)
    text_colors = {
        'driver': '#1E8449',
        'late_support': '#B7950B',
        'suppressor': '#C0392B',
    }
    for i, tick_label in enumerate(ax.get_yticklabels()):
        role = rows[i]['role']
        tick_label.set_color(text_colors.get(role, '#2C3E50'))
        tick_label.set_fontweight('bold')

    # Cascade group labels
    ax.set_xlim(-0.002, None)
    xmax = ax.get_xlim()[1]

    for cid in cascade_order:
        if cid in group_bounds:
            y0, y1 = group_bounds[cid]
            y_mid = (y0 + y1) / 2
            ax.text(xmax * 0.97, y_mid, cascade_labels[cid],
                    ha='right', va='center', fontsize=9, fontweight='bold',
                    fontstyle='italic', color=cascade_dark[cid],
                    path_effects=[PathEffects.withStroke(linewidth=2.5, foreground='white')])

    ax.set_xlabel('Impact score')
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)

    frame_full = FRAME_FULL_NAMES[frame_abbr]
    ax.set_title(
        f'B.  Phase 1 causal impact of event clusters on {frame_abbr} cascades',
        loc='left', fontweight='bold', fontsize=11,
    )

    frame_label = frame_col_name.replace('_', ' ')
    handles = [
        plt.Line2D([0], [0], marker='^', color='w',
                   markerfacecolor=ROLE_COLORS['driver'],
                   markersize=9, label='Driver  (pre-peak, DID > 0)'),
        plt.Line2D([0], [0], marker='D', color='w',
                   markerfacecolor=ROLE_COLORS['late_support'],
                   markersize=8,
                   label=f'Late Support  (post-peak, high {frame_short} affinity)'),
        plt.Line2D([0], [0], marker='v', color='w',
                   markerfacecolor=ROLE_COLORS['suppressor'],
                   markersize=9,
                   label=f'Suppressor  (post-peak, low {frame_short} affinity)'),
    ]
    ax.legend(handles=handles, loc='lower right', frameon=True, fancybox=True,
              edgecolor='#BDC3C7', fontsize=8.5)


# ---------------------------------------------------------------------------
# Generate figure for one frame
# ---------------------------------------------------------------------------
def make_frame_figure(frame_abbr, cc, cluster_map, cascade_map, articles,
                      out_dir, top_n=2):
    # Select top N cascades that have significant clusters, ranked by total_score
    frame_sig = cc[
        (cc['cascade_frame'] == frame_abbr) &
        (cc['role'].isin(['driver', 'late_support', 'suppressor']))
    ]
    cascades_with_impact = set(frame_sig['cascade_id'].unique())

    frame_cascades = sorted(
        [c for c in cascade_map.values()
         if c.frame == frame_abbr and c.cascade_id in cascades_with_impact],
        key=lambda c: c.total_score, reverse=True
    )
    top_cascades = frame_cascades[:top_n]
    top_cascade_ids = [c.cascade_id for c in top_cascades]

    if not top_cascades:
        print(f"  {frame_abbr}: no cascades with significant impact — skipping")
        return None

    # Filter significant clusters to only those targeting selected cascades
    significant = frame_sig[frame_sig['cascade_id'].isin(top_cascade_ids)].copy()
    sig_cascade_ids = sorted(significant['cascade_id'].unique())

    n_clusters = len(significant)
    scores_str = ', '.join(
        f"{c.cascade_id} ({c.total_score:.3f})" for c in top_cascades
    )
    print(f"  {frame_abbr}: top-{top_n} cascades with impact: {scores_str}")
    print(f"    → {n_clusters} significant clusters across "
          f"{len(sig_cascade_ids)} cascades")

    cascade_labels, cascade_colors, cascade_dark = build_cascade_style(top_cascade_ids)

    # Adaptive figure height based on number of clusters
    panel_b_height = max(4, n_clusters * 0.45 + 1.5)
    panel_a_height = 4.5
    fig_height = panel_a_height + panel_b_height + 1.5

    fig, axes = plt.subplots(
        2, 1, figsize=(16, fig_height),
        gridspec_kw={'height_ratios': [panel_a_height, panel_b_height]},
    )
    fig.subplots_adjust(hspace=0.30, left=0.32, right=0.87, top=0.93, bottom=0.05)

    # Dim cascades in top-N that have no significant clusters
    dim_ids = set(top_cascade_ids) - set(sig_cascade_ids)
    plot_timeline(axes[0], frame_abbr, significant, cluster_map, cascade_map,
                  articles, cascade_labels, cascade_colors, cascade_dark,
                  dim_cascade_ids=dim_ids)
    plot_lollipop(axes[1], frame_abbr, significant, cluster_map, cascade_map,
                  articles, cascade_labels, cascade_colors, cascade_dark)

    frame_full = FRAME_FULL_NAMES[frame_abbr]
    fig.suptitle(
        f'Phase 1 Impact Analysis:  Top {top_n} {frame_full} Cascades  (2018)',
        fontsize=14, fontweight='bold', y=0.97,
    )

    fname = f'fig_{frame_abbr.lower()}_phase1_impact_2018.png'
    out_path = out_dir / fname
    fig.savefig(out_path, dpi=200, bbox_inches='tight', facecolor='white')
    plt.close(fig)
    return out_path


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------
def main():
    print("Loading data and computing impact...")
    cc, cluster_map, cascade_map, articles = load_data()

    out_dir = PROJECT_ROOT / 'results' / 'figures'
    out_dir.mkdir(parents=True, exist_ok=True)

    # Check which frames have cascades
    available_frames = sorted(cc['cascade_frame'].unique())
    print(f"Frames with cascade data: {available_frames}")

    saved = []
    for frame_abbr in FRAMES:
        if frame_abbr not in available_frames:
            print(f"  {frame_abbr}: no cascades in results — skipping")
            continue
        path = make_frame_figure(frame_abbr, cc, cluster_map, cascade_map,
                                 articles, out_dir)
        if path:
            saved.append(path)

    print(f"\nDone — {len(saved)} figures saved:")
    for p in saved:
        print(f"  {p}")


if __name__ == '__main__':
    main()
