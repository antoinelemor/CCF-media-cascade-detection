...
class ExhaustiveMetricsCalculator:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.config.setdefault('gpu_threshold', 100)  # Lowered: MPS benefits spectral even at 100 nodes
        ...
        # Metric registry — filtered by whitelist if provided
        self._metric_registry = self._build_metric_registry()
        whitelist = self.config.get('metric_whitelist')
        if whitelist:
            filtered = {}
            for category, funcs in self._metric_registry.items():
                if category in whitelist:
                    allowed = whitelist[category]
                    filtered[category] = {k: v for k, v in funcs.items() if k in allowed}
                # Categories not in whitelist are dropped entirely
            self._metric_registry = filtered
        ...
        # Only log if verbose is enabled
        if self.config.get('verbose', True):
            n_metrics = sum(len(v) for v in self._metric_registry.values())
            logger.info(f"ExhaustiveMetricsCalculator initialized with {len(self._metric_registry)} categories, {n_metrics} metrics"
                       + (f" (filtered by whitelist)" if whitelist else ""))
    ...
    def _compute_all_metrics_parallel(self, G: nx.Graph, use_gpu: bool = False) -> Dict[str, Dict[str, Any]]:
        ...
        # CRITICAL: categories MUST match _metric_registry keys exactly.
        # Bug fix: 'distance' category doesn't exist in registry (diameter is under 'structure'),
        # and transitivity/average_clustering are under 'clustering' not 'structure'.
        expensive_metrics = {
            'spectral': ['eigenvalues', 'spectral_radius', 'energy', 'estrada_index', 'spanning_tree_count'],
            'propagation': ['epidemic_threshold', 'R0', 'influence_maximization', 'cascade_size',
                          'spreading_time', 'complex_contagion'],
            'robustness': ['percolation_threshold', 'attack_robustness', 'failure_robustness',
                         'cascading_failure', 'toughness', 'degeneracy', 'k_core'],
            'structure': ['local_efficiency', 'global_efficiency',
                         'diameter', 'radius', 'center', 'periphery',  # distance-based, O(n*m)
                         'average_path_length',  # O(n*m), was missing → caused deadlock on large graphs
                         'degree_assortativity', 'assortativity', 'rich_club', 's_metric'],
            'community': ['greedy_modularity', 'modularity', 'communities',
                         'louvain', 'label_propagation', 'conductance', 'coverage', 'permanence'],
            'centrality': ['closeness', 'betweenness', 'katz', 'current_flow_betweenness', 'pagerank',
                          'harmonic', 'load', 'eigenvector', 'subgraph', 'percolation'],
            'clustering': ['local_clustering', 'global_clustering', 'transitivity',
                          'average_clustering', 'square_clustering', 'generalized_degree'],
            'connectivity': ['spectral_gap', 'algebraic_connectivity', 'node_connectivity',
                           'edge_connectivity', 'cheeger_constant', 'vertex_expansion', 'edge_expansion'],
        }
        ...
        # PHASE 1: Compute trivial metrics inline (no subprocess needed)
        # After the expensive_metrics fix, only O(n) metrics remain here (degree, density, etc.)
        # Spawning ProcessPoolExecutor for these causes deadlock when called from
        # ThreadPoolExecutor workers (16 threads x 16 processes = 256 processes on macOS spawn).
        logger.info(f"Phase 1: Computing {len(all_metric_tasks)} trivial metrics inline")
        ...
        for category, metric_name, func in all_metric_tasks:
            try:
                ...
                if result is not None:
                    ...
                    logger.debug(f"Computed {category}.{metric_name}")
            except Exception as e:
                logger.debug(f"Failed to compute {category}.{metric_name}: {e}")
                metrics[category][metric_name] = None

        # PHASE 2: Compute expensive metrics using BATCH subprocess + parallel NX
        # Instead of spawning one subprocess per metric (~35 spawns per window),
        # we send ALL NetworKit-supported metrics in a single batch subprocess call.
        # The worker builds the graph once, computes APSP once, and runs all metrics.
        # Non-NetworKit metrics run concurrently via ThreadPoolExecutor.
        if expensive_tasks:
            logger.info(f"Phase 2: Computing {len(expensive_tasks)} expensive metrics (batch mode)")
            ...
            # NetworKit-supported metrics (handled in batch subprocess)
            networkit_supported = {
                # Centrality metrics
            ...
            # Split into NetworKit batch vs NetworkX-only
            nk_batch_tasks = []  # (category, metric_name, func) — sent to batch subprocess
            nx_only_tasks = []   # (category, metric_name, func) — run via NetworkX

            for category, metric_name, func in expensive_tasks:
                if use_networkit and metric_name in networkit_supported:
                    nk_batch_tasks.append((category, metric_name, func))
                else:
                    nx_only_tasks.append((category, metric_name, func))

            nk_metric_names = [name for _, name, _ in nk_batch_tasks]
            # Build category lookup for batch results
            nk_category_map = {name: cat for cat, name, _ in nk_batch_tasks}

            logger.info(
                f"  Batch: {len(nk_metric_names)} NetworKit metrics in 1 subprocess, "
                f"{len(nx_only_tasks)} NetworkX-only metrics in parallel"
            )

            # Run NetworKit batch and NetworkX-only metrics concurrently.
            # The batch subprocess is I/O-bound from main process perspective
            # (waiting on proc.communicate()), so threading works well here.
            from concurrent.futures import ThreadPoolExecutor as _TPE

            batch_results = {}
            nx_results = {}

            with _TPE(max_workers=max(2, len(nx_only_tasks) + 1)) as executor:
                futures = {}

                # Submit NetworKit batch (single subprocess for ALL nk metrics)
                if nk_metric_names:
                    futures[executor.submit(
                        self._compute_networkit_batch_in_subprocess, G, nk_metric_names
                    )] = ('__nk_batch__', '__nk_batch__', None)

                # Submit NetworkX-only metrics concurrently
                for category, metric_name, func in nx_only_tasks:
                    futures[executor.submit(func, G)] = (category, metric_name, func)

                for future in as_completed(futures):
                    category, metric_name, func = futures[future]
                    try:
                        if category == '__nk_batch__':
                            # Unpack batch results
                            batch_results = future.result()
                        else:
                            # Single NX metric result
                            result = future.result()
                            if result is not None:
                                nx_results[(category, metric_name)] = result
                    except Exception as e:
                        if category == '__nk_batch__':
                            logger.error(f"NetworKit batch subprocess failed: {e}")
                        else:
                            logger.error(f"Failed NX metric {category}.{metric_name}: {e}")

            # Merge batch results into metrics dict
            for m_name, m_result in batch_results.items():
                cat = nk_category_map.get(m_name, 'unknown')
                if isinstance(m_result, dict) and '__error__' in m_result:
                    logger.warning(f"NetworKit batch error for {cat}.{m_name}: {m_result['__error__']}")
                    # Try NetworkX fallback for failed batch metrics
                    fallback_func = {name: func for _, name, func in nk_batch_tasks}.get(m_name)
                    if fallback_func:
                        try:
                            m_result = fallback_func(G)
                            logger.debug(f"Fallback OK for {cat}.{m_name}")
                        except Exception as fb_err:
                            logger.error(f"Fallback also failed for {cat}.{m_name}: {fb_err}")
                            m_result = None
                    else:
                        m_result = None
                if m_result is not None:
                    metrics[cat][m_name] = m_result

            # Merge NX-only results
            for (cat, m_name), m_result in nx_results.items():
                metrics[cat][m_name] = m_result
    ...
    def _can_use_networkit(self) -> bool:
        ...
        try:
            ...
            worker_script = os.path.join(os.path.dirname(__file__), 'networkit_worker.py')
    ...
    def _compute_networkit_batch_in_subprocess(self, G: nx.Graph, metric_names: list) -> Dict[str, Any]:
        """Compute MULTIPLE metrics in a single NetworKit subprocess (batch mode).

        Instead of spawning one subprocess per metric, this sends all metric names
        to the worker which builds the graph once, computes APSP once, and runs
        all metrics sequentially inside the warm process.
        ...
        Returns:
            Dict mapping metric_name -> result (or {'__error__': msg} on failure)
        """
        import subprocess
        import pickle
        import sys
        ...
        n_edges = G.number_of_edges()
        logger.info(
            f"Batch subprocess: {len(metric_names)} metrics for graph "
            f"({n_nodes} nodes, {n_edges} edges)"
        )

        # Prepare graph data with batch key
        input_data = {
            'edges': list(G.edges()),
            'nodes': list(G.nodes()),
            'is_directed': G.is_directed(),
            'metric_names': metric_names,  # triggers batch mode in worker
        }

        # Timeout scales with graph size and number of metrics
        # Base: 60s per metric for small graphs, scales up with n_nodes
        disable_timeout = self.config.get('disable_networkit_timeout', False)
        if disable_timeout:
            timeout = None
        else:
            per_metric = min(120, max(30, n_nodes // 10))
            timeout = max(300, per_metric * len(metric_names))
        ...
        try:
            ...
            if timeout is None:
                stdout, stderr = proc.communicate(input=pickle.dumps(input_data))
            else:
                stdout, stderr = proc.communicate(
                    input=pickle.dumps(input_data), timeout=timeout
                )

            if proc.returncode != 0:
                raise RuntimeError(f"NetworKit batch worker failed: {stderr.decode()}")
            ...
            if result.get('success') and result.get('batch'):
                ...
                logger.info(
                    f"Batch subprocess completed: {len(batch_results)} metrics returned"
                )
                return batch_results
            elif result.get('success'):
                # Single-metric response (shouldn't happen in batch mode)
                return {metric_names[0]: result['result']}
            else:
                raise RuntimeError(
                    f"NetworKit batch failed: {result.get('error', 'Unknown')}"
                )

        except subprocess.TimeoutExpired:
            proc.kill()
            raise TimeoutError(
                f"NetworKit batch timed out after {timeout}s for {len(metric_names)} metrics"
            )
        except Exception as e:
            logger.error(f"NetworKit batch subprocess failed: {e}")
            raise
    ...
    def _compute_gpu_metrics(self, G: nx.Graph) -> Dict[str, Dict[str, Any]]:
        ...
        Includes spectral metrics (eigenvalue decomposition) which benefit
        significantly from GPU tensor operations via torch.linalg.eigh().
        ...
        try:
            ...
            try:
                ...
                # Spectral metrics on GPU — eigendecomposition benefits from GPU
                spectral = self._gpu_spectral_metrics(adj_tensor)
                if spectral:
                    metrics['spectral_gpu'] = spectral
    ...
    def _gpu_spectral_metrics(self, adj_tensor: 'torch.Tensor') -> Dict[str, Any]:
        """Compute spectral metrics using torch.linalg on GPU/MPS.

        Uses torch.linalg.eigh() for symmetric eigendecomposition which is
        significantly faster than scipy on GPU for matrices that fit in memory.

        Args:
            adj_tensor: Adjacency matrix as torch tensor (on device)

        Returns:
            Dictionary of spectral metric values
        """
        results = {}
        ...
        try:
            # Symmetrize for undirected interpretation
            sym = (adj_tensor + adj_tensor.T) / 2.0

            # Eigenvalue decomposition on GPU
            # MPS supports eigh on recent PyTorch; falls back to CPU if not
            try:
                eigenvalues = torch.linalg.eigvalsh(sym)
            except RuntimeError:
                # Fallback to CPU for eigendecomposition
                eigenvalues = torch.linalg.eigvalsh(sym.cpu())

            eigenvalues_np = eigenvalues.cpu().numpy()

            # Spectral radius: max |eigenvalue|
            results['spectral_radius'] = float(np.max(np.abs(eigenvalues_np)))

            # Spectral gap: difference between two largest eigenvalues
            sorted_eigs = np.sort(np.abs(eigenvalues_np))[::-1]
            if len(sorted_eigs) >= 2:
                results['spectral_gap'] = float(sorted_eigs[0] - sorted_eigs[1])

            # Graph energy: sum of absolute eigenvalues
            results['energy'] = float(np.sum(np.abs(eigenvalues_np)))

            # Algebraic connectivity: second smallest eigenvalue of Laplacian
            try:
                degree = sym.sum(dim=1)
                laplacian = torch.diag(degree) - sym
                try:
                    lap_eigs = torch.linalg.eigvalsh(laplacian)
                except RuntimeError:
                    lap_eigs = torch.linalg.eigvalsh(laplacian.cpu())
                lap_eigs_np = np.sort(lap_eigs.cpu().numpy())
                if len(lap_eigs_np) > 1:
                    results['algebraic_connectivity'] = float(lap_eigs_np[1])
            except Exception:
                pass  # Skip if Laplacian computation fails

        except Exception as e:
            logger.debug(f"GPU spectral metrics failed: {e}")
