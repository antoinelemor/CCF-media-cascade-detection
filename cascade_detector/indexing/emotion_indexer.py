...
from cascade_detector.core.constants import EMOTION_COLUMNS, EMOTION_LEGACY_MAPPING
...
class EmotionIndexer(AbstractIndexer):
    ...
    def _convert_dates(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        CCF_Database uses native DATE type, while legacy CCF used TEXT MM-DD-YYYY.
        """
        if 'date_converted' not in data.columns:
            # Check if date is already datetime (CCF_Database)
            if pd.api.types.is_datetime64_any_dtype(data['date']):
                data['date_converted'] = data['date']
            else:
                # Try YYYY-MM-DD format first (CCF_Database as string)
                data['date_converted'] = pd.to_datetime(
                    data['date'],
                    format='%Y-%m-%d',
                    errors='coerce'
                )
                # If that fails, try legacy MM-DD-YYYY format
                if data['date_converted'].isna().all():
                    data['date_converted'] = pd.to_datetime(
                        data['date'].str[6:10] + '-' +
                        data['date'].str[0:2] + '-' +
                        data['date'].str[3:5],
    ...
    def _analyze_article_emotion(self, doc_id: str, article_data: pd.DataFrame) -> Dict:
        ...
        # Count emotions - try new column names first, then legacy
        emotion_counts = {}
        for col, emotion_type in self.emotion_cols.items():
            # col is the new column name (tone_positive, etc.)
            actual_col = col
            if col not in article_data.columns:
                # Try to find legacy column name
                for legacy_col, new_col in EMOTION_LEGACY_MAPPING.items():
                    if new_col == col and legacy_col in article_data.columns:
                        actual_col = legacy_col
                        break

            if actual_col in article_data.columns:
                article_data[actual_col] = pd.to_numeric(article_data[actual_col], errors='coerce').fillna(0)
                count = (article_data[actual_col] == 1).sum()
