...
from cascade_detector.core.constants import FRAMES, DATE_FORMAT_DB, DATE_FORMAT_ISO, FRAME_COLUMNS
...
class TemporalIndexer(AbstractIndexer):
    ...
    def build_index(self, data: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        for frame in self.frames:
            # Try new column name first, then legacy
            frame_col = FRAME_COLUMNS.get(frame, f"{frame}_Detection")
            if frame_col not in data.columns:
                # Try legacy column name
                frame_col = f"{frame}_Detection"

            if frame_col not in data.columns:
                logger.warning(f"Frame column for {frame} not found, skipping")
                continue
