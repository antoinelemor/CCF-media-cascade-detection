...
class FastEntityResolver:
    ...
    def __init__(self, 
                 similarity_threshold: float = 0.75,  # Lowered to catch more name variants
    ...
    def _fast_similarity(self, entity1: Tuple, entity2: Tuple) -> float:
        ...
        if data1.get('type') == 'PER':
            ...
            # SPECIAL CASE: Single last name vs full name (e.g., "Trudeau" vs "Justin Trudeau")
            # This is very common in journalism - use statistical approach
            if len(words1) == 1 and len(words2) >= 2:
                # Check if single word matches last name
                if words1[0].lower() == words2[-1].lower():
                    last_name = words1[0].lower()
                    
                    # SPECIFIC RULES FOR PROMINENT POLITICAL FIGURES IN 2020s CONTEXT
                    first_name = words2[0].lower() if len(words2) > 0 else ""
                    
                    if last_name == 'trudeau' and first_name == 'justin':
                        return 0.98  # Very high confidence - this is THE Trudeau of current politics
                    elif last_name == 'poilievre' and first_name == 'pierre':
                        return 0.98  # Pierre Poilievre is the main Poilievre in Canadian politics
                    elif last_name == 'trump' and first_name == 'donald':
                        return 0.98  # Donald Trump is the main Trump in politics
                    elif last_name == 'biden' and first_name == 'joe':
                        return 0.98  # Joe Biden is the main Biden in US politics
                    elif last_name == 'harris' and first_name == 'kamala':
                        return 0.95  # Kamala Harris (slightly lower as Harris is more common surname)
                    
                    # Calculate prominence score based on occurrences
                    occurrences1 = data1.get('occurrences', 0)
                    occurrences2 = data2.get('occurrences', 0)
                    
                    # Statistical approach: if both entities are frequently mentioned,
                    # and one is just the last name, they're likely the same person
                    # This is based on journalistic convention of using full name first,
                    # then last name only in subsequent references
                    
                    # Calculate relative prominence
                    total_occurrences = occurrences1 + occurrences2
                    min_occurrences = min(occurrences1, occurrences2)
                    max_occurrences = max(occurrences1, occurrences2)
                    
                    # If both entities are very high frequency, they're almost certainly the same
                    # (journalists don't use two different prominent people with same last name)
                    if min_occurrences >= 500:
                        # Extremely high confidence - both are very prominent
                        return 0.95
                    elif min_occurrences >= 100:
                        # Very high confidence - both are prominent
                        return 0.92
                    elif total_occurrences >= 100 and min_occurrences >= 30:
                        # High confidence - prominent figure with consistent references
                        return 0.88
                    elif total_occurrences >= 50 and min_occurrences >= 15:
                        # Moderate-high confidence
                        return 0.82
                    elif total_occurrences >= 20:
                        # Moderate confidence - less prominent but still notable
                        return 0.75
                    else:
                        # Low confidence - could be different people with same last name
                        return 0.65
                        
            elif len(words2) == 1 and len(words1) >= 2:
                # Symmetric case
                if words2[0].lower() == words1[-1].lower():
                    last_name = words2[0].lower()
                    
                    # SPECIFIC RULES FOR PROMINENT POLITICAL FIGURES IN 2020s CONTEXT
                    first_name = words1[0].lower() if len(words1) > 0 else ""
    ...
    def _get_name_confidence(self, name1: str, name2: str, words1: List[str], words2: List[str]) -> float:
        ...
        if len(shorter) == 1 and len(longer) >= 2:
            ...
            if longer and last_name == longer[-1].lower():
                ...
                # SPECIFIC RULES FOR PROMINENT POLITICAL FIGURES IN 2020s CONTEXT
                # Check if this is a known prominent figure
                first_name = longer[0].lower() if len(longer) > 0 else ""
                ...
                if last_name == 'trudeau' and first_name == 'justin':
                    ...
                    return 0.95  # Kamala Harris (slightly lower as Harris is more common surname)
                elif last_name == 'ford' and first_name == 'doug':
                    return 0.95  # Doug Ford in Ontario context
                elif last_name == 'smith' and first_name == 'danielle':
                    return 0.92  # Danielle Smith in Alberta context
    ...
    def _merge_cluster_fast(self, cluster_entities: List[Tuple[str, Dict]]) -> Tuple[str, Dict]:
        ...
        # IMPORTANT: For persons, prefer the most complete name as display name and key
        if 'PER:' in canonical_key:
            # Find the entity with the most complete name (most words)
            most_complete_name = canonical_data.get('name', '')
            most_complete_key = canonical_key
            
            for key, data in cluster_entities:
                entity_name = data.get('name', '')
                # Prefer names with more words (e.g., "Justin Trudeau" over "Trudeau")
                if len(entity_name.split()) > len(most_complete_name.split()):
                    most_complete_name = entity_name
                    most_complete_key = key
                # If same length, prefer the one without punctuation issues
                elif len(entity_name.split()) == len(most_complete_name.split()):
                    if ',' not in entity_name and ',' in most_complete_name:
                        most_complete_name = entity_name
                        most_complete_key = key
            
            # Update the merged entity's name to the most complete version
            merged['name'] = most_complete_name
            # Also use the most complete key as canonical
            canonical_key = most_complete_key
